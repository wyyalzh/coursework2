...
void sortComparison(String[] filenames) throws IOException {  
    // 创建输出文件 sortComparison.csv  
    try (PrintWriter writer = new PrintWriter(new FileWriter("sortComparison.csv"))) {  
        // 写入 CSV 文件的表头  
        writer.println("Filename,BubbleSortTime(ms),MergeSortTime(ms)");  

        // 遍历文件数组，分别测量 BubbleSort 和 MergeSort 的执行时间  
        for (String filename : filenames) {  
            // 调用 measureBubbleSort 和 measureMergeSort 方法  
            long bubbleSortTime = measureBubbleSort(filename);  
            long mergeSortTime = measureMergeSort(filename);  

            // 写入结果到 CSV 文件  
            writer.println(filename + "," + bubbleSortTime + "," + mergeSortTime);  
        }  
    }  
}
sortComparison(new String[]{"sort10.txt", "sort1000.txt", "sort10000.txt"})
